...
const BUILD = 'clover-2026-07-07-b'; // bump to verify deploys
    ...
    if(path === 'callback'){
      ...
      if(!code){
        if(merchantId && !(pending && pending.retried)){
          ...
          return redirect(HOSTS[env].authorize + '/oauth/v2/authorize?client_id=' + encodeURIComponent(process.env.CLOVER_APP_ID) + '&response_type=code');
        }
