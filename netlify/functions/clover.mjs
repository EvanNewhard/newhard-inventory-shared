...
const HOSTS = {
  sandbox:    { oauth: 'https://sandbox.dev.clover.com', api: 'https://apisandbox.dev.clover.com' },
  production: { oauth: 'https://www.clover.com',          api: 'https://api.clover.com' },
};
...
async function refreshTokens(auth){
  const host = HOSTS[auth.environment].oauth;
  const r = await fetch(host + '/oauth/v2/refresh', {
    method:'POST', headers:{ 'Content-Type':'application/json' },
    body: JSON.stringify({ client_id: process.env.CLOVER_APP_ID, refresh_token: auth.refreshToken })
  });
  const data = await r.json().catch(()=>({}));
...
export default async (req) => {
  ...
  try {
    ...
    if(path === 'start'){
      ...
      await store().setJSON('pending', { environment: env, at: Date.now() });
      const authorizeUrl = HOSTS[env].oauth + '/oauth/v2/authorize'
        + '?client_id=' + encodeURIComponent(process.env.CLOVER_APP_ID)
    ...
    if(path === 'callback'){
      ...
      if(!code){ return redirect(SITE + '/?clover=error&msg=' + encodeURIComponent('No authorization code returned by Clover.')); }
      try {
