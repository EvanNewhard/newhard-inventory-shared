...
const BUILD = 'clover-2026-07-12-k'; // bump to verify deploys
  ...
  try {
    ...
    if(path === 'push' && req.method === 'POST'){
      ...
      for(const u of updates){
        try {
          // Clover's documented stock-update body includes the item object AND stockCount — sending
          // only {quantity} is accepted but silently does NOT apply. This is the full, correct form.
          const qty = Number(u.quantity);
          await cloverPost(auth, `/v3/merchants/${mId}/item_stocks/${u.id}`, {
            item: { id: u.id },
            quantity: qty,
            stockCount: Math.round(qty)
          });
