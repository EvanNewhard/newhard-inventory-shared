...
const BUILD = 'clover-2026-08-04-itemmods'; // bump to verify deploys
...
// Read an item's CURRENT stock count straight from Clover. Returns a number, or null if the item
// isn't tracking stock / has no stock record (which is exactly the case we need to detect).
async function readItemStock(auth, mId, id){
  try{
    const d = await cloverGet(auth, `/v3/merchants/${mId}/item_stocks/${id}`);
    if(d && typeof d.quantity === 'number') return d.quantity;
    if(d && typeof d.stockCount === 'number') return d.stockCount;
    return null;
  }catch(e){ return null; }
}
...
export default async (req) => {
  ...
  try {
    ...
    if(path === 'connect-token' && req.method === 'POST'){
      ...
      // Validate by reading items (the permission we actually need) — not the merchant record,
      // which would require a separate "Merchants" permission the token may not have.
      const testAuth = { merchantId, accessToken: token, environment: env };
      try {
        await cloverGet(testAuth, `/v3/merchants/${merchantId}/items?limit=1`);
        const auth = {
    ...
    // ---- SALES: read orders for one time window, aggregate by day + item (read-only) ----
    // Query: ?from=<ms>&to=<ms>. Returns { days: { 'YYYY-MM-DD': { itemId: qty } }, totals, orderCount }.
    // The client calls this in bounded windows (recent = a day or two; backfill = a chunk at a time)
    // so we never blow past the function time limit or Clover's rate limits.
    if(path === 'sales' && req.method === 'GET'){
      ...
      while(guard++ < 50){
        ...
        for(const o of els){
          orderCount++;
          const day = new Date(o.createdTime || to).toISOString().slice(0,10);
          const lis = (o.lineItems && o.lineItems.elements) || [];
          for(const li of lis){
            const iid = li.item && li.item.id;
            if(!iid) continue;
            // Countable retail items usually come back as one line item per unit; use unitQty when present.
            let qty = 1;
            if(typeof li.unitQty === 'number' && li.unitQty > 0) qty = li.unitQty;
            days[day] = days[day] || {};
    ...
    // ---- ITEM-MODIFIERS: read the flavors (modifiers) Clover has for one item ----
    // GET /api/clover/item-modifiers?id=<cloverItemId>  → { flavors:[{name}], groups:[...] }
    // Used by the modifier-items page to auto-populate flavors when you add an item.
    if(path === 'item-modifiers' && req.method === 'GET'){
      const auth = await validAuth();
      if(!auth) return json({ error:'Not connected to Clover.' }, 400);
      const itemId = (url.searchParams.get('id') || '').trim();
      if(!itemId) return json({ error:'Missing item id.' }, 400);
      try {
        const data = await cloverGet(auth, `/v3/merchants/${auth.merchantId}/items/${encodeURIComponent(itemId)}?expand=modifierGroups.modifiers`);
        const groups = (data.modifierGroups && data.modifierGroups.elements) || [];
        const flavors = [];
        const seen = new Set();
        groups.forEach(g=>{
          const mods = (g.modifiers && g.modifiers.elements) || [];
          mods.forEach(m=>{
            const nm = String(m.name||'').trim();
            if(nm && !seen.has(nm)){ seen.add(nm); flavors.push({ name: nm }); }
          });
        });
        return json({ ok:true, itemId, itemName: data.name || '', flavorCount: flavors.length, flavors,
          groups: groups.map(g=>({ name:g.name||'', count:((g.modifiers&&g.modifiers.elements)||[]).length })) });
      } catch(e){
        return json({ error: 'Could not read modifiers for that item: ' + String(e.message||e).slice(0,160) }, 502);
      }
    }

    // ---- RAW: inspect exactly what Clover reports for items matching a name (diagnostic) ----
    // GET /api/clover/raw?name=blueberry   → shows Clover's own code/sku fields, unmodified.
    if(path === 'raw' && req.method === 'GET'){
      const auth = await validAuth();
      if(!auth) return json({ error:'Not connected to Clover.' }, 400);
      const mId = auth.merchantId;
      const needle = String(url.searchParams.get('name')||'').toLowerCase();
      if(!needle) return json({ error:'Pass ?name=<part of item name>' }, 400);
      const found = [];
      let offset=0; const limit=200;
      for(let g=0; g<20; g++){
        const page = await cloverGet(auth, `/v3/merchants/${mId}/items?limit=${limit}&offset=${offset}`);
        const els = (page && page.elements) || [];
        els.forEach(it=>{
          if(String(it.name||'').toLowerCase().includes(needle)){
            found.push({ id: it.id, name: it.name, code: it.code === undefined ? '(absent)' : it.code, sku: it.sku === undefined ? '(absent)' : it.sku });
          }
        });
        if(els.length < limit) break;
        offset += limit;
      }
      return json({ ok:true, build:BUILD, count:found.length, items:found });
    }
    ...
    if(path === 'pull' && req.method === 'GET'){
      ...
      const out = items.map(it => ({
        ...
        // Product code MIRRORS Clover's "code" field exactly. SKU is a DIFFERENT Clover field and is
        // never read as a product code — doing so made cleared codes linger and put non-Esh items on
        // the Esh order.
        productCode: (it.code != null ? String(it.code).trim() : ''),
        ...
        hasStock: !!(it.itemStock && (it.itemStock.quantity != null || it.itemStock.stockCount != null)),
        // Clover flags the website uses to decide "tracked" the SAME way an export upload does:
        priceType: it.priceType || '',            // 'FIXED' | 'VARIABLE' | 'PER_UNIT'
        isRevenue: (it.isRevenue !== false),        // non-revenue items (gift cards etc.) have isRevenue:false
        hidden: !!it.hidden,
      }));
    ...
    if(path === 'push' && req.method === 'POST'){
      ...
      for(const u of updates){
        const qty = Math.round(Number(u.quantity));
        let ok = false, lastErr = '';
        for(let attempt=0; attempt<3 && !ok; attempt++){
          try {
            // Make the write actually stick the FIRST time: turn on stock tracking (idempotent — a
            // no-op if it's already on), then set the count. Clover drops stock writes on untracked
            // items, so enabling first is what makes every item sync instead of half of them.
            try { await cloverPost(auth, `/v3/merchants/${mId}/items/${u.id}`, { itemStock: { trackStock: true } }); } catch(e){ /* may already be on / not needed */ }
            await cloverPost(auth, `/v3/merchants/${mId}/item_stocks/${u.id}`, { item:{id:u.id}, quantity: qty, stockCount: qty });
            ok = true;
          } catch(e){
            lastErr = String(e.message||e).slice(0,200);
            await new Promise(r=>setTimeout(r, 400*(attempt+1)));   // brief backoff, then quietly retry
          }
        }
        results.push({ id: u.id, ok, error: ok?null:lastErr });
        await new Promise(res=>setTimeout(res, 90)); // gentle throttle for rate limits
      }
