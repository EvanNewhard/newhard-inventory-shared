...
export default async (req) => {
  ...
  if (req.method === 'GET') {
    try {
      ...
      // A missing blob (null) is a genuine fresh install → empty at version 0.
      ...
      // A real read ERROR must NOT look like "no data" — return 500 so the client knows the load
  ...
  if (req.method === 'POST') {
    try {
      ...
      try {
        ...
        // data. Refuse the save and tell the client to retry — never write blind.
      ...
      // defense against the "reopened and lost everything" bug — no client can wipe the data.
